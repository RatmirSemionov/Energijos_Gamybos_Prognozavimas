...
BEST_PARAMS_REGIONS = {
    ...
    "combined_dataset": {
        ...
        "attention_head_size": 4,
        "lstm_layers": 1,
        "dropout": 0.3994914774641232,
        "learning_rate": 5.0576554262777065e-05,
        "gradient_clip_val": 0.1,
        "batch_size": 128,
    },
}
...
BEST_PARAMS = build_best_params("combined_dataset")
...
KNOWN_REALS = [
    ...
    "weekend", "month", "wday",
    # Apšvitos požymiai
    "GTI", "dir_frac", "gti_over_ghi", "GTI_max24",
    "gti_rel_max",
    # Sąveikos kintamieji
    ...
    "WeatherID501", "WeatherID502", "WeatherID600", "WeatherID601",
    "WeatherID800", "WeatherID801", "WeatherID802",
    "WeatherID803", "WeatherID804",
    # Ilgesni vėlavimai (12h, 24h) - pasiekiami ir žinomi ateityje
    "GTI_lag12", "GTI_lag24", "Temp_lag12", "Temp_lag24",
    "WindSpeed_lag12", "WindSpeed_lag24", "Humidity_lag12",
    "Humidity_lag24", "Clouds_lag12", "Clouds_lag24",
    "dir_frac_lag12", "dir_frac_lag24", "gti_over_ghi_lag12",
    "gti_over_ghi_lag24",
    # Slenkantys statistiniai rodikliai (12h, 24h langai)
    "GTI_ma12", "GTI_sd12", "GTI_ma24", "GTI_sd24", "Temp_ma12",
    "Temp_ma24", "WindSpeed_ma12", "WindSpeed_ma24", "Humidity_ma12",
    "Humidity_ma24", "Clouds_ma12", "Clouds_ma24", "dir_frac_ma12",
    "dir_frac_ma24", "gti_over_ghi_ma12", "gti_over_ghi_ma24",
]

UNKNOWN_REALS = [
    # Prognozuojamas kintamasis
    "Energy",
    # Trumpesni vėlavimai (1-6h) - naudojami tik enkoderiui
    "GTI_lag01", "GTI_lag02", "GTI_lag03", "GTI_lag06",
    "Temp_lag01", "Temp_lag02", "Temp_lag03",
    "Temp_lag06", "WindSpeed_lag01", "WindSpeed_lag02",
    "WindSpeed_lag03", "WindSpeed_lag06", "Humidity_lag01",
    "Humidity_lag02", "Humidity_lag03", "Humidity_lag06",
    "Clouds_lag01", "Clouds_lag02", "Clouds_lag03",
    "Clouds_lag06", "dir_frac_lag01", "dir_frac_lag02",
    "dir_frac_lag03", "dir_frac_lag06", "gti_over_ghi_lag01",
    "gti_over_ghi_lag02", "gti_over_ghi_lag03",
    "gti_over_ghi_lag06",
    # Trumpesni slenkantys rodikliai (3h, 6h langai)
    "GTI_ma03", "Temp_ma03", "WindSpeed_ma03", "Humidity_ma03",
    "Clouds_ma03", "dir_frac_ma03", "gti_over_ghi_ma03",
    "GTI_sd03", "GTI_ma06", "Temp_ma06", "WindSpeed_ma06",
    "Humidity_ma06", "Clouds_ma06", "dir_frac_ma06",
    "gti_over_ghi_ma06", "GTI_sd06",
]
...
# -------------------------------------------------------
# Duomenų paruošimas globaliam modeliui
# -------------------------------------------------------
def prepare_df(df_exogenous: pd.DataFrame) -> pd.DataFrame:
    df = df_exogenous.copy()
    df["Energy"] = df["Energy"].clip(lower=0.0001)
    df["ts_local"] = pd.to_datetime(df["ts_local"])

    if df["ts_local"].dt.tz is not None:
        df["ts_local"] = df["ts_local"].dt.tz_localize(None)

    # Rūšiuojama pirmiausia pagal laiką, kad indeksas didėtų nuosekliai einant per eilutes
    df = df.sort_values(["ts_local", "plant_id"]).reset_index(drop=True)
    df["group"] = df["plant_id"]

    # Sukuriamas unikalus valandinis indeksas bendrai laiko ašiai
    min_ts = df["ts_local"].min()
    df["time_idx"] = ((df["ts_local"] - min_ts).dt.total_seconds() // 3600).astype(int)
    df = df.sort_values(["time_idx", "group"]).reset_index(drop=True)
...
def train_and_evaluate(
    ...
    unique_times = np.sort(df["time_idx"].unique())
    total_hours = len(unique_times)
    test_hours_count = int(total_hours * test_fraction)
    train_hours_count = total_hours - test_hours_count

    train_end_idx = unique_times[train_hours_count - 1]

    train_df = df[df["time_idx"] <= train_end_idx].copy()
    test_df_check = df[df["time_idx"] > train_end_idx]

    print(f"\nBendras unikalių valandų skaičius: {total_hours}")
    print(f"Mokymo eilutės: {len(train_df)}")
    print(f"Testavimo eilutės: {len(test_df_check)}")
    print(f"Mokymas baigiasi ties laiko indeksu: {train_end_idx}\n")
    ...
    val_start = max(0, train_end_idx - best_params["val_length_hours"])
    validation_ds = TimeSeriesDataSet.from_dataset(
        training_ds,
        df[df["time_idx"] <= train_end_idx].copy(),
    ...
    print("Final model saved → tft_solar_model.ckpt\n")
    ...
    results_df, metrics = _run_sliding_inference(
        ...
        train_end=train_end_idx, n_total=unique_times[-1],
...
def _run_sliding_inference(
    ...
    ts_map = df.set_index(["time_idx", "group"])["ts_local"].to_dict()
    ...
    for run_num, pred_start in enumerate(midnight_starts, 1):
        ...
        if len(window_df) < ((encoder_length // 2) + prediction_length) * window_df["group"].nunique():
            continue
        ...
        try:
            ...
            loader = window_ds.to_dataloader(train=False, batch_size=64, num_workers=0)
            ...
            group_names = window_ds.decoded_index["group"].values

            for i, plant_name in enumerate(group_names):
                p10 = preds[i, :, 0]
                median = preds[i, :, 1]
                p90 = preds[i, :, 2]

                for hour in range(prediction_length):
                    target_idx = int(d_idx[i, hour])
                    target_ts = ts_map.get((target_idx, plant_name))

                    if target_ts is None or (target_ts, plant_name) not in results_df.index:
                        continue

                    if pd.isna(results_df.loc[(target_ts, plant_name), "D1"]):
                        results_df.loc[(target_ts, plant_name), "D1"] = median[hour]
                        results_df.loc[(target_ts, plant_name), "D1_p10"] = p10[hour]
                        results_df.loc[(target_ts, plant_name), "D1_p90"] = p90[hour]
    ...
    for k, v in global_metrics.items():
        print(f"  {k:20s}: {f'{v:.2f}%' if 'WMAPE' in k else f'{v:.4f}'}")

    print("\n================ Metrikos pagal elektrine ================")
    for plant in valid["group"].unique():
        df_p = valid[valid["group"] == plant]
        plant_metrics = evaluate_model(
            y_true=df_p["actual"].values,
            y_pred=df_p["D1"].values,
            ts_col="timestamp",
            df=df_p
        )
        print(f"\n{plant.upper()}:")
        for k, v in plant_metrics.items():
            print(f"  {k:20s}: {f'{v:.2f}%' if 'WMAPE' in k else f'{v:.4f}'}")
...
if __name__ == "__main__":
    ...
    df_exogenous = pq.read_table("apjungtos_duomenys.parquet").to_pandas()


    # Optuna optimizavimas
    def objective(trial):
